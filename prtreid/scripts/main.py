...
import prtreid
from prtreid.tools.extract_part_based_features import extract_reid_features
...
from prtreid.scripts.default_config import (
...
def build_datamanager(cfg):
    if cfg.data.type == 'image':
        return prtreid.data.ImageDataManager(**imagedata_kwargs(cfg))
    else:
        return prtreid.data.VideoDataManager(**videodata_kwargs(cfg))


def build_engine(cfg, datamanager, model, optimizer, scheduler, writer, engine_state):
    if cfg.data.type == 'image':
        if cfg.loss.name == 'softmax':
            engine = prtreid.engine.ImageSoftmaxEngine(
                datamanager,
                model,
                optimizer=optimizer,
                scheduler=scheduler,
            ...
            engine = prtreid.engine.ImageTripletEngine(
                datamanager,
                model,
                optimizer=optimizer,
            ...
            engine = prtreid.engine.ImagePartBasedEngine(
                datamanager,
                model,
                optimizer=optimizer,
        ...
        if cfg.loss.name == 'softmax':
            engine = prtreid.engine.VideoSoftmaxEngine(
                datamanager,
                model,
                optimizer=optimizer,
                scheduler=scheduler,
            ...
            engine = prtreid.engine.VideoTripletEngine(
                datamanager,
                model,
                optimizer=optimizer,
                margin=cfg.loss.triplet.margin,
